...
* RaspberryPI - streaming from Raspberry Pi Camera Module.
...
The cam2web application supports HTTP digest authentication and allows to configure who can view the camera or change its settings (like can be viewed by everyone and configured only users, or viewed by users and configured by admins only, etc).
